...
\begin{enumerate}[leftmargin=*,itemsep=1pt,topsep=2pt]
    ...
    \item Comparing the two networks' Grad-CAM saliency maps~\cite{selvaraju2017} to see whether they attend to the same cytological structures (Section~\ref{sec:interpretability}).
\end{enumerate}
...
\code{A2SDNet121} has no explicit self-attention mechanism (only SE channel recalibration acting on local convolutional features), so a common, architecture-agnostic lens is needed for a fair comparison. We use \textbf{Grad-CAM}~\cite{selvaraju2017} on the last spatial feature map before global average pooling in \emph{both} networks --- the same diagnostic Zhang \emph{et al.}\ use in their own Fig.~14 to compare DenseNet121 against their SE-augmented model. For every sampled image we also compute the \textbf{normalized Shannon entropy} of the Grad-CAM heatmap as a simple focus score (lower = more spatially concentrated; higher = more diffuse).

\begin{table}[H]
\centering\footnotesize
\begin{tabular}{@{}p{0.42\linewidth}cc@{}}
\toprule
\textbf{Comparison} & \textbf{A2SDNet121} & \textbf{SwinAD2Net} \\
\midrule
SIPaKMeD, 5-class (10 imgs) & 0.980 & 0.992 \\
Combined, binary (6 imgs) & 0.957 & 0.974 \\
\bottomrule
\end{tabular}
\caption{Mean normalized Grad-CAM entropy (lower = more concentrated). A2SDNet121 is consistently more concentrated than SwinAD2Net across both comparisons.}
\label{tab:entropy}
\end{table}

Across both comparisons (Table~\ref{tab:entropy}), \textbf{A2SDNet121 produces more spatially concentrated Grad-CAM maps}, usually centered tightly on the nucleus/cell body --- consistent with Zhang \emph{et al.}'s own claim that their SE-augmented model "pays more attention to key regions (nuclei)." SwinAD2Net's attention is measurably more diffuse, sometimes spreading into the background. This tracks the architectural difference: SE re-weights \emph{channels} globally but still acts on local convolutional features, while Swin's window attention explicitly mixes information across spatial positions, which shows up as a less spatially-sharp saliency map even when the underlying prediction is more accurate (Table~\ref{tab:mainresults}).

\begin{figure}[H]
    \centering
    \includegraphics[width=\linewidth]{sipakmed_multiclass_metaplastic_example.png}
    \caption{SIPaKMeD, Metaplastic cell. Both models correctly focus on the cell interior; A2SDNet121's hotspot is tighter.}
    \label{fig:cam_metaplastic}
\end{figure}

\begin{figure}[H]
    \centering
    \includegraphics[width=\linewidth]{combined_binary_abnormal_example.png}
    \caption{Combined dataset, abnormal cell. A2SDNet121 concentrates on the nucleus/cytoplasm; SwinAD2Net's attention bleeds into the surrounding background (top-right corner).}
    \label{fig:cam_abnormal}
\end{figure}

\begin{figure}[H]
    \centering
    \includegraphics[width=\linewidth]{combined_binary_normal_example.png}
    \caption{Combined dataset, normal cell --- a counter-example where the trend reverses: SwinAD2Net's attention is a tight hotspot directly on the nucleus, while A2SDNet121 spreads across nearly the whole cytoplasm. Table~\ref{tab:entropy}'s entropy numbers are averages; per-image behavior varies, and this figure is a reminder not to over-read a small qualitative sample as a universal architectural rule.}
